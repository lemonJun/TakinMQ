...
public class FileRecords extends AbstractRecords implements Closeable {
    ...
    public TimestampAndOffset searchForTimestamp(long targetTimestamp, int startingPosition) {
        for (LogEntry shallowEntry : shallowEntriesFrom(startingPosition)) {
            ...
            if (shallowRecord.timestamp() >= targetTimestamp) {
                ...
                throw new IllegalStateException(String.format("The message set (max timestamp = %s, max offset = %s" + " should contain target timestamp %s, but does not.", shallowRecord.timestamp(), shallowEntry.offset(), targetTimestamp));
            }
        }
        return null;
    }
    ...
    public static class LogEntryPosition {
        ...
        public String toString() {
            return "LogEntryPosition(" + "offset=" + offset + ", position=" + position + ", size=" + size + ')';
        }
    }
