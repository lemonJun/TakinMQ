...
public class CleanerThread extends Thread {
    ...
    private void init() {
        ...
        cleaner = new Cleaner(threadId.getAndIncrement(), new SkimpyOffsetMap((int) Math.min(config.dedupeBufferSize / config.numThreads, Integer.MAX_VALUE), config.hashAlgorithm), /*ioBufferSize = */config.ioBufferSize / config.numThreads / 2, /*maxIoBufferSize = */config.maxMessageSize, /*dupBufferLoadFactor = */config.dedupeBufferLoadFactor, /*throttler = */throttler, /*time = */time);
    ...
    public void logStats(int id, String name, long from, long to, CleanerStats stats) {
        ...
        String message = String.format("%n\tLog cleaner %d cleaned log %s (dirty section = [%d, %d])%n", id, name, from, to) + String.format("\t%,.1f MB of log processed in %,.1f seconds (%,.1f MB/sec).%n", mb(stats.bytesRead), stats.elapsedSecs(), mb(stats.bytesRead / stats.elapsedSecs())) + String.format("\tIndexed %,.1f MB in %.1f seconds (%,.1f Mb/sec, %.1f%% of total time)%n", mb(stats.mapBytesRead), stats.elapsedIndexSecs(), mb(stats.mapBytesRead) / stats.elapsedIndexSecs(), ((double) 100 * stats.elapsedIndexSecs()) / stats.elapsedSecs()) + String.format("\tCleaned %,.1f MB in %.1f seconds (%,.1f Mb/sec, %.1f%% of total time)%n", mb(stats.bytesRead), stats.elapsedSecs() - stats.elapsedIndexSecs(), mb(stats.bytesRead) / (stats.elapsedSecs() - stats.elapsedIndexSecs()), 100 * ((double) (stats.elapsedSecs() - stats.elapsedIndexSecs()) / stats.elapsedSecs())) + String.format("\tStart size: %,.1f MB (%,d messages)%n", mb(stats.bytesRead), stats.messagesRead) + String.format("\tEnd size: %,.1f MB (%,d messages)%n", mb(stats.bytesWritten), stats.messagesWritten) + String.format("\t%.1f%% size reduction (%.1f%% fewer messages)%n", 100.0 * (1.0 - ((double) stats.bytesWritten) / stats.bytesRead), 100.0 * (1.0 - ((double) stats.messagesWritten) / stats.messagesRead));
        logger.info(message);
    }
}
