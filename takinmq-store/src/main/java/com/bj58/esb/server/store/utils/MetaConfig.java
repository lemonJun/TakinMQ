...
public class MetaConfig extends Config implements Serializable, MetaConfigMBean {
    ...
    private static MetaConfig metaConfig = new MetaConfig();
...
//    private int maxSegmentSize = 1 * 10 * 1024;
...
//            throw new MetamorphosisServerStartupException("Parse configuration failed,path=" + path, e);
            try {
				throw new Exception("Parse configuration failed,path=" + path, e);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
        }
...
//            throw new MetamorphosisServerStartupException("Parse configuration failed,path=" + this.path, e);
            try {
				throw new Exception("Parse configuration failed,path=" + this.path, e);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
        }
