...
public class MemoryRecords extends AbstractRecords {
    ...
    private static FilterResult filterTo(Iterable<ByteBufferLogEntry> fromShallowEntries, LogEntryFilter filter, ByteBuffer destinationBuffer) {
        ...
        for (ByteBufferLogEntry shallowEntry : fromShallowEntries) {
            ...
            if (writeOriginalEntry) {
                ...
                MemoryRecordsBuilder builder = builderWithEntries(slice, shallowRecord.timestampType(), shallowRecord.compressionType(), shallowRecord.timestamp(), retainedEntries);
    ...
    private Iterator<LogEntry> deepIterator(boolean ensureMatchingMagic, int maxMessageSize) {
        return new RecordsIterator(new ByteBufferLogInputStream(buffer.duplicate(), maxMessageSize), false, ensureMatchingMagic, maxMessageSize);
    }
