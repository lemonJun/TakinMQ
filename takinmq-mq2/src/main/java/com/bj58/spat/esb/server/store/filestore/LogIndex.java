...
public class LogIndex {
    private static final Log logger = LogFactory.getLog(LogIndex.class);
    ...
    private AtomicInteger size = new AtomicInteger(); /*计算队列长度*/
    ...
    /**
     * totalLength:  总长度 = 消息长度 + 间隔字节
     * offset：                发送失败字节数
     * mmap：                     相应LogEntity类的副本
     * return value：消息的个数
     * @throws FileFormatException 
     * */
    public void goBackPosition(int backLength, MappedByteBuffer mmap) throws IOException, FileFormatException {
        int pos = 0;
        int msgLen;

        while (pos < backLength) {
            mmap.position(readerPosition - backLength + pos);
            if ((msgLen = mmap.getInt()) == 0) {
                break;
            }
            pos = pos + msgLen + 4;
            incrementSize();
        ...
        if (backLength > 0) {
            readerPosition = readerPosition - backLength;
        }
    }

    public void recalculateSize(MappedByteBuffer mmap) {
        int pos = 20;
        int msgLen;

        while (pos < readerPosition) {
            mmap.position(pos);
            if ((msgLen = mmap.getInt()) == 0) {
                break;
            }
