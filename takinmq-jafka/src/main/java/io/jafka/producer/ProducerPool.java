...
public class ProducerPool<V> implements Closeable {
    ...
    public void send(ProducerPoolData<V> ppd) {
        ...
        if (sync) {
            ...
            if (producer == null) {
                throw new UnavailableProducerException("Producer pool has not been initialized correctly. " + "Sync Producer for broker " + ppd.partition.brokerId + " does not exist in the pool");
            }
    ...
    private void syncSend(List<ProducerPoolData<V>> poolData) {
        ...
        for (Map.Entry<Integer, List<ProducerRequest>> e : topicBrokerIdData.entrySet()) {
            ...
            if (producer == null) {
                throw new UnavailableProducerException("Producer pool has not been initialized correctly. " + "Sync Producer for broker " + e.getKey() + " does not exist in the pool");
            }
