...
public class NodeApiVersions {
    ...
    public static NodeApiVersions create(Collection<ApiVersion> overrides) {
        ...
        for (ApiKeys apiKey : ApiKeys.values()) {
            ...
            if (!exists) {
                apiVersions.add(new ApiVersion(apiKey.id, ProtoUtils.oldestVersion(apiKey.id), ProtoUtils.latestVersion(apiKey.id)));
            }
        }
    ...
    public short usableVersion(ApiKeys apiKey) {
        ...
        if (usableVersion == API_NOT_ON_NODE)
            ...
            throw new UnsupportedVersionException("The broker is too old to support " + apiKey + " version " + ProtoUtils.oldestVersion(apiKey.id));
        else if (usableVersion == NODE_TOO_NEW)
            throw new UnsupportedVersionException("The broker is too new to support " + apiKey + " version " + ProtoUtils.latestVersion(apiKey.id));
        else
