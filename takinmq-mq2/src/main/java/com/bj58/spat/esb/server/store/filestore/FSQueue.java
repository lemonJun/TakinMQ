...
public class FSQueue {
     ...
     */
    public LogEntity createLogEntity(String dbpath, LogIndex db, int fileNumber) throws IOException, FileFormatException {
    ...
    public byte[] readNextAndRemove() throws IOException, FileFormatException {
        ...
        try {
            try {
                ...
                readerHandle.close();
                if (readerHandle.getCurrentFileNumber() == writerHandle.getCurrentFileNumber()) {
                    System.out.println("delete write");
                    writerHandle.close();
                }
                deleteFileRunner.addDeleteFile(path + fileSeparator + filePrefix + "data_" + deleteNum + ".idb");
                // 更新下一次读取的位置和索引
        ...
        try {
            o: while (n > 0) {
                try {
                    ...
                    for (int i = 0; i < n; i++) {
                        if (readerHandle.getEndPosition() != -1 && readerHandle.getReaderPosition() >= readerHandle.getEndPosition()) {
                            throw new FileEOFException("file eof");
                        }
                        if (readerHandle.getReaderPosition() >= readerHandle.getWriterPostion()) {
                            System.out.println(readerHandle.getReaderPosition() + "   " + readerHandle.getReaderPosition());
                            readerHandle.putReaderPosition(readerHandle.getWriterPostion());
                            break o;
                        }
                        readerHandle.mappedByteBuffer.position(readerHandle.getReaderPosition());
                    ...
                    readerHandle.putReaderPosition(readerHandle.getReaderPosition());
                    n = n - readnfinished;
                } catch (FileEOFException e) {
                    n = n - readnfinished;
                    int deleteNum = readerHandle.getCurrentFileNumber();
    ...
    public void readGoBack(int backLength) throws IOException, FileFormatException {
        ...
        if (backLength > (readposition - 20)) {
            ...
            if (currFileNumber > 0) {
                ...
                readerHandle = createLogEntity(path + fileSeparator + filePrefix + "data_" + currFileNumber + ".idb", db, currFileNumber);
