...
public class ListOffsetRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<ListOffsetRequest> {
        ...
        public ListOffsetRequest build() {
            ...
            if (version < minVersion) {
                throw new UnsupportedVersionException("Cannot create a v" + version + " ListOffsetRequest because " + "we require features supported only in " + minVersion + " or later.");
            }
            if (version == 0) {
                if (offsetData == null) {
                    if (partitionTimestamps == null) {
                        ...
                        for (Map.Entry<TopicPartition, Long> entry : partitionTimestamps.entrySet()) {
                            offsetData.put(entry.getKey(), new PartitionData(entry.getValue(), 1));
                        }
        ...
        public String toString() {
            ...
            bld.append("(type=ListOffsetRequest").append(", replicaId=").append(replicaId);
            if (offsetData != null) {
    ...
    public static final class PartitionData {
        ...
        public String toString() {
            StringBuilder bld = new StringBuilder();
            bld.append("{timestamp: ").append(timestamp).append(", maxNumOffsets: ").append(maxNumOffsets).append("}");
            return bld.toString();
        }
    }
    ...
    private ListOffsetRequest(int replicaId, Map<TopicPartition, ?> targetTimes, short version) {
        ...
        Map<String, Map<Integer, Object>> topicsData = CollectionUtils.groupDataByTopic((Map<TopicPartition, Object>) targetTimes);
