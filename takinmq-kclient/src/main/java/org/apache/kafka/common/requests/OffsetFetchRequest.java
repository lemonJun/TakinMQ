...
public class OffsetFetchRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<OffsetFetchRequest> {
        ...
        public String toString() {
            ...
            bld.append("(type=OffsetFetchRequest, ").append("groupId=").append(groupId).append(", partitions=").append(Utils.join(partitions, ",")).append(")");
            return bld.toString();
        }
    }
    ...
    public OffsetFetchResponse getErrorResponse(Errors error) {
        ...
        if (versionId < 2) {
            for (TopicPartition partition : this.partitions) {
                responsePartitions.put(partition, new OffsetFetchResponse.PartitionData(OffsetFetchResponse.INVALID_OFFSET, OffsetFetchResponse.NO_METADATA, error));
            }
        }
    ...
    public static OffsetFetchRequest parse(ByteBuffer buffer, int versionId) {
        return new OffsetFetchRequest(ProtoUtils.parseRequest(ApiKeys.OFFSET_FETCH.id, versionId, buffer), (short) versionId);
    }
