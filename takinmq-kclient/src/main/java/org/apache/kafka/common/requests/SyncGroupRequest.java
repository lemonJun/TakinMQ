...
public class SyncGroupRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<SyncGroupRequest> {
        ...
        public String toString() {
            ...
            bld.append("(type=SyncGroupRequest").append(", groupId=").append(groupId).append(", generationId=").append(generationId).append(", memberId=").append(memberId).append(", groupAssignment=").append(Utils.join(groupAssignment.keySet(), ",")).append(")");
            return bld.toString();
        }
    }
    ...
    public AbstractResponse getErrorResponse(Throwable e) {
        ...
        switch (versionId) {
            case 0:
                return new SyncGroupResponse(Errors.forException(e).code(), ByteBuffer.wrap(new byte[] {}));
            default:
                throw new IllegalArgumentException(String.format("Version %d is not valid. Valid versions for %s are 0 to %d", versionId, this.getClass().getSimpleName(), ProtoUtils.latestVersion(ApiKeys.SYNC_GROUP.id)));
        }
    }
    ...
    public static SyncGroupRequest parse(ByteBuffer buffer, int versionId) {
        return new SyncGroupRequest(ProtoUtils.parseRequest(ApiKeys.SYNC_GROUP.id, versionId, buffer), (short) versionId);
    }
